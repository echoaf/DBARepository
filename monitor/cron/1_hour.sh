...
printLog "[$f_name]======begin deal 1_hour======" "$normal_log"
values=$(getCommonScript "1_hour" "$local_ip")

for value in $(echo "$values")
do
    person_value=$(getPersonScript "$value" "1_hour" "$local_ip")
    ...
    getDealStatus "$local_ip" "$basename" "1 hour"
    if (($?==0));then
